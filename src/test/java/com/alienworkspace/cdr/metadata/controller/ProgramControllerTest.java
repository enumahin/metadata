...
public class ProgramControllerTest {
    ...
    public void testUpdateNonExistingProgram() throws Exception {
        // Arrange
        ProgramDto programDto = programDtoBuilder.build();
        when(programService.updateProgram(1, programDto)).thenThrow(new ResourceNotFoundException("Program not found"));
    ...
    public void testDeleteNonExistingProgram() throws Exception {
        ...
        RecordVoidRequest recordVoidRequest = RecordVoidRequest.builder()
                .voidReason("Test Void Reason")
                .build();
        doThrow(new ResourceNotFoundException("Program not found")).when(programService).deleteProgram(1,recordVoidRequest);
        ...
        result.andDo(print())
                .andExpect(status().isNotFound())
                .andExpect(jsonPath("$.errorCode").value(404))
                .andExpect(jsonPath("$.errorMessage").value("Program not found"));
    }
}
